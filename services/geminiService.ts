...
export async function askSupport(question: string) {
  try {
    const response = await ai.models.generateContent({
      ...
      contents: question,
      config: {
        systemInstruction: "Anda adalah asisten bantuan TEXA-TOOLS, marketplace AI nomor 1 di Indonesia. Tugas Anda adalah membantu pengguna (kreator digital) dengan masalah teknis injeksi sesi, pertanyaan langganan, dan rekomendasi tool dalam Bahasa Indonesia yang ramah, santun, dan sangat solutif. Gunakan istilah gaul digital yang sopan."
      }
    });
    return response.text;
  } catch (error) {
